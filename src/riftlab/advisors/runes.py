...
Rune page data is stored in data/champions/<ChampName>.json — edit those files
...
# ---------------------------------------------------------------------------
# Rune definitions (ID + metadata) — used for display only
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Rune page data lives in data/champions/<ChampName>.json
# Access via: from champion_loader import get_rune_pages
# ---------------------------------------------------------------------------
...
def _normalize_champ(name: str) -> str:
    """Resolve champion name via champion_loader aliases."""
    return resolve_name(name) or name


def pick_rune_page(champion: str, ctx: dict) -> dict | None:
    champion = _normalize_champ(champion)
    pages = get_rune_pages(champion)
    for page in pages:
        if any(ctx.get(a) for a in page.get("anti", [])):
            continue
        if "always" in page.get("conditions", []) or all(ctx.get(c) for c in page.get("conditions", [])):
            return page
...
def main():
    ...
    other_pages = [p for p in get_rune_pages(_normalize_champ(my_champ)) if p["name"] != page["name"]]
    if other_pages:
        console.print()
